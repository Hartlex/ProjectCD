...
namespace SunStructs.RuntimeDB.Parsers
{
    public class RankOptionParser
    {
        ...
        private readonly Dictionary<ItemType,Dictionary<AttrType,RankOption>> _rankOptionDictionary=new();
            public Dictionary<ItemType, Dictionary<AttrType, RankOption>>  ParseAllOptions(string dataFolderPath)
            {
        ...
        private void ParseLine(string line)
        {
            ...
            if(!_rankOptionDictionary.TryGetValue(rankInfo.Type,out var dictionary))
                _rankOptionDictionary.Add(rankInfo.Type,new Dictionary<AttrType, RankOption>());
#if DEBUG
            Logger.Instance.LogOnLine($"Loaded RankOption[{rankInfo.AttrType}]");
#endif
            dictionary?.Add(rankInfo.AttrType,rankInfo);
        }
    }
}
